...
let view = ColorView(frame: NSRect(x: 0, y: 0, width: 1000, height: 500))
...
func visualize(_ points: [Point]) {
    let pairs = zip(view.subviews, points)

    for p in pairs {
        p.0.setFrameOrigin(NSPoint(x: p.1.x, y: p.1.y))
    }
}
...
rows.forEach { _ in
    view.addSubview(PointView(frame: NSRect(x: 0, y: 0, width: 2, height: 2)))
}
var currentPoints = rows.map({ Point($0) })


let context = Context(currentPoints)
let points = context.run()
